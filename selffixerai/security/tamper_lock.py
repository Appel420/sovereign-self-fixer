        ...
        try:
            with open(self.chain_file, "r") as f:
                for line in f:
                    ...
                    if entry.get("content_hash") != self._hash(content):
                        return False
            return True
        except Exception:
            return False

    async def seal_state_to_tpm(self, content: str) -> Optional[bytes]:
        if not self.tpm or not await self.tpm.is_tpm_present():
            return None
        return await self.tpm.seal_with_pcr_policy(content.encode())

    async def unseal_state_from_tpm(self, sealed_blob: bytes) -> Optional[str]:
        if not self.tpm or not await self.tpm.is_tpm_present():
            return None
        result = await self.tpm.unseal_with_pcr_policy(sealed_blob)
        return result.decode() if result else None

    async def recover_from_nv_index(self) -> Optional[str]:
        if not self.tpm or not await self.tpm.is_tpm_present():
            return None
        return await self.tpm.read_hash_chain_from_nv()